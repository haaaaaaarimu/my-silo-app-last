...
def make_block(prefix, rect_start, circ_start, df):
    ...
    for row in range(2):
        ...
        for col in range(7):
            ...
            rects_html += f'<div class="rect-item"><div class="text-box"><span class="p-name" style="color:{p_c}">{name}</span><span class="p-qty" style="color:{q_c}">{qty}</span><span class="p-code">{code}</span></div></div>'
    ...
    for r_idx, y_pos in enumerate([0, 160, 320]):
        ...
        for c_idx in range(6):
            ...
            circles_html += f'<div class="circle-item" style="top: {y_pos}px; left: {x_pos}px;"><div class="text-box"><span class="p-name" style="color:{p_c}">{name}</span><span class="p-qty" style="color:{q_c}">{qty}</span><span class="p-code">{code}</span></div></div>'

    return rects_html + circles_html + '</div></div>'
...
if raw_data.strip():
    ...
    try:
        ...
        <style>
            ...
            .circle-item {{ position: absolute; width: 100px; height: 100px; border-radius: 50%; border: 2.5px solid #000; background: #fff; display: flex; align-items: center; justify-content: center; transform: translate(-50%, -50%); }}
